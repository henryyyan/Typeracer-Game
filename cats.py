...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    def temp(paragraph):
        temp1 = remove_punctuation(paragraph).lower().split()
...
def accuracy(typed, source):
    ...
    # BEGIN PROBLEM 3
    j = 0
    n, k = len(typed_words), len(source_words)
    if n == k and n == 0:
        return 100.0
    elif (n == 0 and k != 0) or (k == 0 and n != 0):
        return 0.0
    for i in range(n):
        if i <= k - 1:
...
def time_per_word(words, times_per_player):
    ...
    for i in range(l):
        for n in range(len(times_per_player[i]) - 1):
            time[i] = time[i] + [times_per_player[i][n + 1] - times_per_player[i][n]]
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    fastword = [[] for i in range(len(player_indices))]

    for i in word_indices:
        fasttime = time(match, 0, i)
        fastplayer = 0
        for player in player_indices:
            if time(match, player, i) < fasttime:
                fasttime = time(match, player, i)
                fastplayer = player
        fastword[fastplayer].append(get_all_words(match)[i])
    return fastword
    # END PROBLEM 10
